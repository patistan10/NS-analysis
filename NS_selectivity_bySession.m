...
home=pwd;
cd ..
NS = load(sprintf('%s_NatScenes_dataOut.mat',expID));
gdir = sprintf('H:/ProcessedDataArchive/Pati/NatScene2/%s_suite2p_processed/processed_suite2p/%s_analysis/Gratings',expID,expID);
cd(gdir)
G = load(sprintf('%s_Grating_dataOut.mat',expID));
cd(home);

stims = NS.dataOut.totalNumStimuli;
cells_picked = [1:NS.dataOut.totalNumCells];

%get average responses for all neurons
current = NS.dataOut.responseMatrix_1;
current2 = NS.dataOut.isRemovedBlock;
for c = 1:NS.dataOut.totalNumCells
    c_resps = squeeze(current(:,c,:));
    c_bad = squeeze(current2(:,c,:));
    c_resps(c_bad==1) = nan;
    c_resps_avg = nanmean(c_resps,1);
    NS.dataOut.stats.global.response_ACTUAL_avg_vals(c,:) = c_resps_avg;        
end
dataOut = NS.dataOut;
save(sprintf('%s_NatScenes_dataOut_mod.mat',expID),'dataOut');
clear dataOut
...
save('NS_selectivity_data.mat','AUC_all','NS_selec')

%% get BW of all grating cells and compare with selectivity
cd(gdir)
cd Gauss_fit
Fits = load('gaussFit_results.mat');
cd(home)
...
saveas(gca,'cumulative_dist_sharp_broad_NSI.fig')
saveas(gca,'cumulative_dist_sharp_broad_NSI.png')
