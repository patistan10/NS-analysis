...
home = pwd;
cd(gratingsdir)
gratings = load(sprintf('%s_Grating_dataOut.mat',expID));
cd(nsdir)
...
grating_index = zeros(1,gratings.dataOut.totalNumCells);
grating_index(grating_cells) =1;
ns_cells = NS.dataOut.stats.global.responsive_cells_p001_fdr_average';
ns_index = zeros(1,NS.dataOut.totalNumCells);
ns_index(ns_cells) = 1;
...
for g = 1:5
    ...
    [AllFold_AllBins,selected_cells,bins_accuracy] = NatScene_decoding_ver8_for20(expID,type,selected_cells,folds);
